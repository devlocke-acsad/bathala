/**
 * RoadCarver — carves road networks into terrain grids.
 *
 * A reusable building block: generates road paths using Bresenham's line drawing.
 * Use this inside your algorithm when you want connected road corridors.
 *
 * How it works:
 *   1. Generate major roads through the chunk center
 *   2. Optionally add secondary roads for variety
 *   3. Clean up isolated road segments
 *
 * @example
 *   import { RoadCarver } from '../../toolbox/RoadCarver';
 *   const roads = new RoadCarver({ roadDensity: 0.7 });
 *   const connections = roads.generate(grid, 20, 20, rng);
 *
 * @module RoadCarver
 */
...
export interface RoadCarverConfig {
...
/** @deprecated Use RoadCarverConfig */
export type RoadNetworkConfig = RoadCarverConfig;

const DEFAULTS: Required<RoadCarverConfig> = {
    roadDensity: 0.7,
...
export class RoadCarver {
    private readonly config: Required<RoadCarverConfig>;

    constructor(config: RoadCarverConfig = {}) {
...
/** @deprecated Use RoadCarver */
export const RoadNetworkAlgorithm = RoadCarver;
