/**
 * SkywardCitadelChunkAdapter — IChunkGenerator for Act 3: The Skyward Citadel.
 *
 * Uses a **zone-based system** to create Filipino-style village clusters
 * placed on cloud platforms and ethereal citadel terrain:
 *   DENSE      — Tightly packed houses with narrow alleys (citadel core)
 *   TRANSITION — Houses tapering into forest (citadel edge)
...
import { SkywardCitadelAlgorithm, DEFAULT_CITADEL_PARAMS, CitadelLayoutParams } from './SkywardCitadelAlgorithm';
...
export enum CitadelChunkType {
    /** Dense housing — tightly packed, narrow alleys, citadel core */
    DENSE = 'dense',
...
function densePreset(cfg: typeof ZONE_DEFAULTS): CitadelLayoutParams {
    return {
...
function forestPreset(cfg: typeof ZONE_DEFAULTS): CitadelLayoutParams {
    return {
        ...DEFAULT_CITADEL_PARAMS,
...
export class SkywardCitadelChunkAdapter implements IChunkGenerator {
    ...
    /**
     * Determine what kind of citadel chunk this position produces.
     * Uses Chebyshev distance to the nearest citadel center.
     */
    getChunkType(chunkX: number, chunkY: number): {
        type: CitadelChunkType;
        ...
        if (dist <= this.cfg.denseRadius) {
            return { type: CitadelChunkType.DENSE, bias: null };
        }
        if (dist <= this.cfg.denseRadius + this.cfg.transitionRadius) {
            return {
                type: CitadelChunkType.TRANSITION,
                bias: { x: -Math.sign(dx) || 0, y: -Math.sign(dy) || 0 },
            };
        }
        return { type: CitadelChunkType.FOREST, bias: null };
    }

    /** Generate terrain using zone-appropriate citadel parameters. */
    generate(chunkX: number, chunkY: number, rng: SeededRandom): RawChunk {
        const { type, bias } = this.getChunkType(chunkX, chunkY);

        let params: CitadelLayoutParams;
        switch (type) {
            case CitadelChunkType.DENSE:
                params = densePreset(this.cfg);
                break;
            case CitadelChunkType.TRANSITION:
                params = transitionPreset(this.cfg, bias ?? { x: 0, y: 0 });
                break;
            case CitadelChunkType.FOREST:
            default:
                params = forestPreset(this.cfg);
                break;
        }

        const gen = new SkywardCitadelAlgorithm();
