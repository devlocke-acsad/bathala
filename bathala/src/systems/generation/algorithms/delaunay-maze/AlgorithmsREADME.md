...
- **Primary chunk adapter** – `DelaunayMazeChunkAdapter.ts` produces corridor layouts with pathfinding-aware carving.
...
Key tunables live at the top of `DelaunayMazeChunkAdapter.ts`, enabling quick iteration on density, spacing, and path heuristics.
...
- **Delaunay chunk adapter** – `levelSize`, `regionCount`, `minRegionDistance`, A* cost weights, and waypoint jitter live at the top of `DelaunayMazeChunkAdapter.ts` for rapid balancing.
