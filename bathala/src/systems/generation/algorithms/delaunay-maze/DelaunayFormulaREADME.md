...
This document expands on the implementations in `DelaunayMazeChunkAdapter.ts`, `MazeOverworldGenerator.ts`, and the surrounding utilities under `utils/MazeGeneration/`. It captures the formulas, heuristics, and computational flow that drive the current Delaunay-based corridor generator used for overworld chunks.
...
2. **Seed resolution**: `RandomUtil.getChunkSeed(chunkX, chunkY, globalSeed)` produces `seed`, aligning RNG usage across nodes and future PRNG injection in `DelaunayMazeChunkAdapter`.
3. **Layout generation**: `generateCorridorChunk` configures `DelaunayMazeChunkAdapter` (`levelSize`, `regionCount`, `minRegionDistance`) and calls `generateLayout()` to obtain `IntGrid` corridors.
...
3. **Future integration**: once `DelaunayMazeChunkAdapter` accepts an injected RNG, region point sampling and waypoint decisions will use the same seeded stream, guaranteeing identical corridor layouts per chunk.
