import { Point, Edge, PathNode, IntGrid } from './DataStructures';
import { SeededRandom } from './types';

/**
 * Proper Delaunay-like triangulation implementation
 * Creates natural connectivity similar to the external tool
 */
class SimpleDelaunay {
    static triangulate(points: Point[]): Edge[] {
        if (points.length < 3) {
        ...
        // Sort points to ensure deterministic triangulation
        points.sort((a, b) => a.x !== b.x ? a.x - b.x : a.y - b.y);

        // Create triangulation by connecting nearby points
        for (let i = 0; i < points.length; i++) {
            const p1 = points[i];
            const neighbors: Array<{point: Point, index: number, distance: number}> = [];
            ...
            for (let j = 0; j < points.length; j++) {
                ...
                neighbors.push({point: p2, index: j, distance});
            }

            // Sort by distance
            neighbors.sort((a, b) => a.distance - b.distance);

            // Connect to closest neighbors (adaptive based on point count)
            const maxConnections = Math.min(
                Math.max(2, Math.floor(Math.sqrt(points.length))), 
                neighbors.length
            );
            ...
            for (let k = 0; k < maxConnections; k++) {
                ...
                if (!edgeSet.has(edgeKey)) {
                    edgeSet.add(edgeKey);
                    edges.push(new Edge(p1, neighbor.point));
                }
            }
        }

        // Add some longer connections for variety (like external tool does)
        const numLongConnections = Math.floor(points.length * 0.3);
        for (let i = 0; i < numLongConnections && i < points.length - 1; i++) {
            const p1 = points[i];
            const p2 = points[(i + Math.floor(points.length / 2)) % points.length];
            const edgeKey = `${Math.min(i, (i + Math.floor(points.length / 2)) % points.length)},${Math.max(i, (i + Math.floor(points.length / 2)) % points.length)}`;
 ...
 * -------------------
 * Generates dungeon-like layouts using Delaunay triangulation and A* pathfinding.
 * 
 * Pipeline:
 ...
 * 4. For each edge: findPath() -> Multi-waypoint A* pathfinding
 * 5. fixDoubleWidePaths() -> Enforce no 2x2 PATH blocks invariant
 * 6. Dead-end remediation (external) -> Tiered cleanup system
 */
export class DelaunayMazeGenerator {
    ...
    // Tile types (matching external tool)
    public readonly PATH_TILE = 1;            // Carved corridor
    public readonly REGION_TILE = 2;          // Background / un-carved area  
    public readonly REGION_CENTER_TILE = 3;   // Optional marker for region seeds
    ...
    // Captured unique edges from triangulation
    public edges: Edge[] = [];

    /**
     * Primary entry: build and post-process a grid layout.
     */
    generateLayout(rng: SeededRandom, chunkSize: number = 50): IntGrid {
        this.levelSize = [chunkSize, chunkSize];
        this.edges = [];

        // 1) Region seeds with spacing guard
        const points = this.generateRegionPoints(rng);
        ...
        // 5) Extract edges from triangulation
        const edges = SimpleDelaunay.triangulate(points);
        ...
        for (const edge of edges) {
            ...
            const path = this.findPath(point1, point2, intGrid, rng);
        ...
        // 8) Enforce structural invariant (no 2x2 PATH squares)
     ...
     */
    private generateRegionPoints(rng: SeededRandom): Point[] {
        const points: Point[] = [];
        let attempts = 0;

        while (points.length < this.regionCount && attempts < this.MAX_REGION_POINT_ATTEMPTS) {
            attempts++;
            const centerX = Math.floor(rng.next() * this.levelSize[0]);
            const centerY = Math.floor(rng.next() * this.levelSize[1]);
     ...
     */
    private findPath(start: [number, number], end: [number, number], intGrid: IntGrid, rng: SeededRandom): [number, number][] | null {
        // First try to find path with waypoints for more interesting shapes
        const waypoints = this.generateWaypoints(start, end, rng);
     ...
     */
    private generateWaypoints(start: [number, number], end: [number, number], rng: SeededRandom): [number, number][] {
        ...
        const pathStyle = pathStyles[Math.floor(rng.next() * pathStyles.length)];

        if (pathStyle === 'L_shape') {
            // Create L-shaped path
            if (rng.next() < this.L_SHAPE_FIRST_AXIS_PROB) {
            ...
            const offsetX = Math.floor(rng.next() * (this.STEP_MIDPOINT_JITTER * 2 + 1)) - this.STEP_MIDPOINT_JITTER;
            const offsetY = Math.floor(rng.next() * (this.STEP_MIDPOINT_JITTER * 2 + 1)) - this.STEP_MIDPOINT_JITTER;
     ...
     */
    public findPathSegment(start: [number, number], end: [number, number], intGrid: IntGrid): [number, number][] | null {
     ...
     */
    public fixDoubleWidePaths(intGrid: IntGrid): void {
    ...
    /**
     * Choose a PATH tile inside a 2x2 block to revert to WALL.
    ...
    public inBounds(pos: [number, number]): boolean {
        return (
     ...
     */
    public wouldCreateDoubleWideAt(pos: [number, number], intGrid: IntGrid): boolean {
        const [x, y] = pos;
